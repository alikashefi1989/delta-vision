...
> extends React.Component<p, S, SS> {
    handleError(handleErrorObj: IHandleError): IHandleErrorResolve {
        const defaults: IHandleError = {
            // error: {},
            notify: true,
            type: 'ui',
            // body: '', // Localization.msg.ui.error_occurred,
        ...
        if (!obj.body) {
            if (obj.error) {
                if (obj.error.data?.meta?.message) {
                    obj.body = obj.error.data.meta.message;
                    // if (obj.error.data.meta.message) { // .msg
                    //     obj.body = this.translateErrorMsg(obj.error.data) || Localization.msg.ui.error_occurred;
                    // } else {
        ...
        if (obj.notify) {
            ...
            const toastOptions = Object.assign(obj.toastOptions || {}, {
                autoClose: obj.timeout,
                render: obj.body,
            });
            if (toastOptions.toastId && toast.isActive(toastOptions.toastId)) {
                toast.update(
                    toastOptions.toastId,
                    this.getNotifyConfig(toastOptions)
                );
            } else {
    ...
    toastNotify(
        notifyBody: string,
        config: ToastOptions,
        toastType: 'info' | 'success' | 'error' | 'warn'
    ) {
        if (config.toastId && toast.isActive(config.toastId)) {
            toast.update(
                config.toastId,
                this.getNotifyConfig({ ...config, ...{ render: notifyBody } })
            );
        } else {
    ...
    protected timestamp_to_date(timestamp: number) {
        try {
            ...
            console.error('baseCMP method timestamp_to_date:', e);
        }
    }
