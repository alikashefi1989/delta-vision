...
export interface IProps<T> extends Omit<Props<any>, 'name'>, Omit<AsyncProps<any>, 'name'>, IFilterFormElementBaseProps<T> {
    control: APP_FILTER_FORM_CONTROL.FILTER_ASYNC_SELECT;
    // loadOptions: (inputValue: string, limit?: number, skip?: number) => Promise<Array<{ label: string, value: any }>>;
    optionsLabelCreator: (value: any) => string;
    pureValue2MongoValue?: (value: any) => any;
}
...
type IFilterAsyncSelectValue = { [key in OPERATOR]?: any; }
interface ISelectValue { label: string; value: OPERATOR }

export class FilterAsyncSelect<T> extends FilterFormElementBase<T, IProps<T>, IState>{
    ...
    private asyncValue: any = undefined;

    private convertOuter2Inner(v?: IFilterAsyncSelectValue): { operator: ISelectValue, value: { label: string, value: any } | null; } {
        // debugger
        ...
        if (this.operators.includes(op) && op in v) {
            ...
            return {
                ...
                value: {
                    label: this.props.optionsLabelCreator(this.asyncValue),
    ...
    private convertInner2Outer(o?: ISelectValue, v?: { label: string, value: any } | null): IFilterAsyncSelectValue | undefined {
        const operator = o ? o.value : this.selectValueOnEmpty.value;
        this.selectValueOnEmpty = o ? o : this.selectValueOnEmpty;
        this.asyncValue = (v === undefined || v === null) ? undefined : v.value;
        if (!v || v === null) return;
        return { [operator]: this.props.pureValue2MongoValue ? this.props.pureValue2MongoValue(v.value) : v.value };
    }
    ...
    protected fieldRender() {
        ...
        return <>
            <Field name={name} {...rest}>
                {({ form, field }: FieldProps<IFilterAsyncSelectValue, T>) => {
                    ...
                    return <div className="d-flex filter-elements-holder">
                        ...
                        <AsyncSelect
                            ...
                            defaultOptions
                            value={fieldValue}
                            // loadOptions={(inputValue: string) => this.props.loadOptions(inputValue)}
                            onChange={(v: any) => {
                                if (!v) { v = undefined; }
                                //  debugger
