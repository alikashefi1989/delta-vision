...
export interface IProps<T> extends Partial<Omit<ReactDatePickerProps, 'name' | 'onChange'>>, IFilterFormElementBaseProps<T> {
    control: APP_FILTER_FORM_CONTROL.FILTER_DATE_PICKER;
    placeholder?: string;
    onChange?: (v: IFilterDatePickerValue | undefined) => void;
}
...
type IFilterDatePickerValue = { [key in OPERATOR]?: number }
interface ISelectValue { label: string; value: OPERATOR }

export class FilterDatePicker<T> extends FilterFormElementBase<T, IProps<T>, IState>{

    private opreatorOptions: Array<ISelectValue> = [
        { label: "before", value: OPERATOR.$lt },
        { label: "between", value: OPERATOR.$between },
        { label: "after", value: OPERATOR.$gt },
    ];
    operators = Object.values(OPERATOR);
    private selectValueOnEmpty = this.opreatorOptions[0];


    private convertOuter2Inner(v?: IFilterDatePickerValue): { operator: ISelectValue, value: { from?: number, to?: number } } {
    ...
    private convertInner2Outer(o?: ISelectValue, v?: { from?: number, to?: number } | null): IFilterDatePickerValue | undefined {
        // debugger
    ...
    private finalCheck(obj: IFilterDatePickerValue | undefined): IFilterDatePickerValue | undefined {
        // debugger
        if (obj === undefined) {
            return
        }
        let finalObj: IFilterDatePickerValue = {};
    ...
    protected fieldRender() {
        ...
        return <>
            <Field name={name} {...rest}>
                {({ form, field }: FieldProps<IFilterDatePickerValue, T>) => {
                    ...
                    return <div className="d-flex filter-elements-holder">
                        ...
                        />
                        {
                            (fieldOperator.value === OPERATOR.$gt || fieldOperator.value === OPERATOR.$between)
                                ?
                                <div className={"app-datepicker app-datepicker-sm w-100 " + (fieldOperator.value === OPERATOR.$between ? "mr-2" : '')}>
                                    <DatePicker
                                        {...field}
                                        {...rest}
                                        selected={fieldValue.from === undefined ? null : new Date(fieldValue.from)}
                                        value={fieldValue.from === undefined ? '' : new Date(fieldValue.from).toLocaleDateString("en-US")}
                                        onChange={(v: Date | null | any) => {
                                            fieldValue.from = (v === null || v === undefined) ? undefined : new Date((v as number)).getTime()
                                        ...
                                        todayButton="today"
                                        isClearable={true}
                                        placeholderText={fieldOperator.value === OPERATOR.$between ? Localization.from : Localization.date}
                                    />
                                </div>
                                :
                                undefined
                        }
                        {
                            (fieldOperator.value === OPERATOR.$lt || fieldOperator.value === OPERATOR.$between)
                                ?
                                <div className={"app-datepicker app-datepicker-sm w-100 " + (fieldOperator.value === OPERATOR.$between ? "ml-2" : '')}>
                                    <DatePicker
                                        {...field}
                                        {...rest}
                                        selected={fieldValue.to === undefined ? null : new Date(fieldValue.to)}
                                        value={fieldValue.to === undefined ? '' : new Date(fieldValue.to).toLocaleDateString("en-US")}
                                        onChange={(v: Date | null | any) => {
                                            fieldValue.to = (v === null || v === undefined) ? undefined : new Date((v as number)).getTime()
                                            const filterVal = this.convertInner2Outer(fieldOperator, fieldValue);
                                            // debugger
                                            if (this.props.onChange) this.props.onChange(filterVal);
                                            else setFieldValue(name as string, this.finalCheck(filterVal));
                                        ...
                                        todayButton="today"
                                        isClearable={true}
                                        placeholderText={fieldOperator.value === OPERATOR.$between ? Localization.to : Localization.date}
                                    />
                                </div>
                                :
                                undefined
